...
\begin{document}
    ...
    The application proved successful in connecting to the API, and thereby controlling the printer.
    The device surprised by showing no problems in performance, and proved to be a capable monitoring device.
    \\\\
    The screensize required a small design, but all MVP features was implementable.
    It is unsure wheter any more features would be feasable to implement, but that would require a completely new design
    due to the large use of icons.
    \\\\
    If the application was to be published a new design would be needed.
    The usage of icons is problematic as none hold any real desciption.
    Also a help button or tutorial features would be required for new users.
