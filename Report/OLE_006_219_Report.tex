...
\begin{document}
    ...
    The TizenPrint application was sucsessfully setup with the planned architecture and proved successful in connecting to the API,
    thereby being able to controll the printer.
    ...
    Due to unforeseen problems during the development all MVP features was not implemented before the submission deadline,
    although all proved to be possible from testing the API with the Postman tool and JavaScript console.
    \\\\
    The screensize of the Galaxy watch required a small design, but all MVP features proved to be implementable whitin a reasonable 
    userfriendly design.
    Though it is unsure wheter any more features would be feasable to implement, and that would require a completely new design
    due to the overuse of icons.
    This usage of icons is problematic as the icons does not hold any textual desciption, requireing a help page or tutorial feature
    for new users.
    \\\\
    Lastely implementation of the accessibility features from Tizen OS required too much work and was not specified in the MVP.
    It is still worth mentioning that any application publicly provided would normally be under regulations requireing such
    features.
