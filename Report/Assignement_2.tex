...
\begin{document}
    ...
    I try to develop an application for the \textit{"Samsung Galaxy Watch"} using their proprietary development-studio
    \textit{Tizen studio} to allow control of my personal FDM-3D-Printer.

    \mysection{Motivation}
    The Tizen app-store for the Samsung Galaxy Watch was lackluster at best.
    The list off available applications is short, and there was nothing available for use with octoprint.
    Owning a personal open-source 3D-Printer, there are great advantages to having a simple, easily-accessible interface
    to control and boost safety of the device.
    \\\\
    After developing a smaller application towards a different REST-API, the task looked manageable within the given
    timeframe, and the language not too complicated.
    \newpage
    ...
    standard.
    \\\\
    The REST or Restful part only means that the host/server does not hold any state for you.
    \mycitation{restfulapi}
    \\\\
    \textbf{NOTE:} This section is to be expanded
    \begin{itemize}
        \item Separation of frontend / backend and its advantages
        \item Allowing same backend for several devices, applications and interfaces
        \item Open APIs allow 3d-party integrations
    \end{itemize}
